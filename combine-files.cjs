...
const argv = minimist(process.argv.slice(2), {
  ...
  default: {
    ...
    'file-types':
      ',.js,.json,.yml,.yaml,.vue,.html,Dockerfile,.cjs,.md,.ts,.prisma,.sh',
    'output-file': 'combined_content.txt',
  },
})

const baseDir = path.resolve(argv.path)
const fileTokens = argv['file-types']
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean)
...
function createGitignoreFilter(dir) {
  ...
  return (relPath) =>
    !patterns.some(({ raw, re }) => {
      if (raw === 'node_modules' && relPath.includes('node_modules'))
        return true
      if (raw === 'package-lock.json' && relPath.endsWith('package-lock.json'))
        return true
...
function getFiles(startDir, tokens) {
  ...
  console.log(`🔍 Scanning for files in ${startDir}...`)
  console.log(`   Matching tokens: ${tokens.join(', ')}`)
  ...
  while (stack.length) {
    ...
    for (const entry of fs.readdirSync(cur)) {
      ...
      if (stat.isDirectory()) {
        if (isAllowed(path.relative(startDir, full).replace(/\\/g, '/')))
          stack.push(full)
        continue
      }
...
function concatFiles(files, dest) {
  ...
  if (fs.existsSync(promptFile)) {
    ...
    fs.appendFileSync(dest, `${promptContent}\n\n CODE:\n\n`)
  }
